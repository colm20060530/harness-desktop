...
function Show-Status {
    ...
    foreach ($c in ($channels.Keys | Sort-Object)) {
        ...
        $desktopKey = if ($c -eq 'glm' -or $c -eq 'glm-thinking') { Get-SkillConfigValue 'apiKey' } else { '' }
        $status = if ($missing.Count -eq 0) { 'configured' } elseif ($desktopKey) { 'configured (via Harness Desktop config.json)' } else { 'dormant (missing: ' + ($missing -join ', ') + ')' }
        Write-Output ("- {0} [{1}]: {2}" -f $c, $info.name, $status)
    }
    Write-Output ("- desktop config.json: {0}" -f $(if (Get-SkillConfigValue 'apiKey') { 'configured' } else { 'absent' }))
