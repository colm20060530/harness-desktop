...
v4.0 起新增**内置生图能力**：在对话里用自然语言说“画一只小猫”“把这张图的背景换成星空”，即可自动生成或改图，出图后自动用系统看图程序打开，全程无需指定技能或模型。
...
3. [内置生图能力（v4.0 主打）](#内置生图能力v40-主打)
4. [功能特性](#功能特性)
5. [系统要求](#系统要求)
6. [适用人群](#适用人群)
7. [下载与安装](#下载与安装)
8. [快速开始](#快速开始)
9. [接入免费模型（示例一：Agnes AI）](#接入免费模型示例一agnes-ai)
10. [接入免费模型（示例二：智谱 GLM）](#接入免费模型示例二智谱-glm)
11. [数据与配置（含备份）](#数据与配置含备份)
12. [从源码构建](#从源码构建)
13. [项目结构](#项目结构)
14. [工作原理](#工作原理)
15. [自定义与打造你的专属桌面端](#自定义与打造你的专属桌面端)
...
v4.0 起，Harness Desktop 把**文生图与图改图**能力完全内置：不挑聊天模型，用自然语言就能出图，出图后自动打开，体验与原生功能一致。

### 自然语言触发，无需指定技能或模型

- **文生图**：直接说“画一只小猫”“生成一张海报”“帮我做个头像”“给文章配个图”，当前任意聊天模型都会自动调用内置生图技能完成出图；
- **图改图**：上传一张图说“把背景换成星空”“把照片变成动漫风格”“给这只猫戴上帽子”，应用会先识别原图内容，再结合修改要求生成新图，表面上实现“图生图”；
- 内置大量触发说法（`ds-image-skill/references/triggers.md`），用户口语化表达即可命中，不需要记住任何技能名或模型名。

### 一键配置生图 API（设置 → 模型）

生图调用基元律动（TokenRhythm）的 OpenAI 兼容生图接口，支持 `qwen-image-2.0`（默认）与 `wan2.7-image` 两个模型。v4.0 在「设置 → 模型」内置「生图能力」卡片：

- 只需填入基元律动 API Key（[前往基元律动获取](https://tokenrhythm.studio/account/keys)），点击「保存」即可；若已配置基元律动聊天模型，会自动复用同一个 Key，无需重复填写；
- 该 Key **仅供生图技能调用**，不会出现在模型列表中，也不会被当作聊天模型使用；
- **计费说明**：生图按张计费（约 ¥0.2/张），需要基元律动账户余额充足；识图能力使用永久免费模型，无需充值；
- 保存后自动写入技能配置，重启 / 升级 / 覆盖更新均永久生效；凭据清除时技能配置同步清空。

### 出图后自动打开

- 生成或改图成功后，应用会自动用系统默认看图程序打开图片，同时在对话里显示保存路径；
- 图片保存到工作目录 `images\` 文件夹，可直接在文件管理器中找到。

> 密钥安全：识图与生图的 API Key 只保存在应用数据目录（`dsh-home\.credentials.yaml` 与技能 `config.json`），不会写入源码、不会被打包进安装包，也不会上传到任何仓库。
...
| 生图 | 内置 ds-image-skill 生图技能，文生图 / 图改图自然语言触发，出图后自动打开；设置 → 模型 一键配置基元律动 Key（按张计费）；识图与生图 Key 均不写入源码 |
...
| 文件 | 说明 |
| --- | --- |
| `Harness-Desktop-Setup-4.0.0.exe` | 安装版：向导安装、创建桌面/开始菜单快捷方式、可在「应用和功能」中卸载 |
...
1. 双击 `Harness-Desktop-Setup-4.0.0.exe`；
...
```powershell
Harness-Desktop-Setup-4.0.0.exe /S /D=D:\HarnessDesktop
```
...
大版本更新：**内置生图能力（文生图 / 图改图）** 正式发布。

新增（生图能力完全内置）：

- **技能内置**：ds-image-skill 随应用自动安装到数据目录，任意聊天模型开箱即可生图，无需手动安装技能；
- **自然语言触发**：说“画一只小猫”“生成一张海报”“把这张图的背景换成星空”“把照片变成动漫风格”即可自动触发生图 / 改图，无需指定技能名或模型名；内置大量触发说法库（`references/triggers.md`）；
- **图改图组合流程**：上传图片要求改图时，自动先识别原图内容，再合并修改要求生成新图，表面上实现图生图；
- **出图后自动打开**：生成或改图成功后，应用自动用系统默认看图程序打开图片，同时在对话中显示保存路径（由应用主进程可靠打开，不受沙箱 shell 限制）；
- **一键配置**：设置 → 模型 新增「生图能力」卡片，填入基元律动 API Key 即保存（附官方获取链接）；已配置基元律动聊天模型时自动复用同一 Key；该 Key 仅用于生图技能，不会出现在模型列表；
- **计费说明**：生图按张计费（约 ¥0.2/张），需基元律动余额充足；识图能力使用永久免费模型，无需充值；
- **密钥安全**：识图与生图 API Key 仅保存在应用数据目录，不写入源码、不打包进安装包、不上传仓库，并加入 `.gitignore` 兜底；
- **UI 完善**：原生下拉框（选择提供商 / API 协议等）改为玻璃外观，选项文字常显，不再依赖悬停；
- **默认观感回归**：修复默认值自检对旧标记的读取，首次启动深色 + 视频壁纸 + 玻璃参数（模糊 16 / 磨砂 13 / 视频模糊 6 / 视频亮度 20）校验更准确。

- 安装包：`Harness-Desktop-Setup-4.0.0.exe`（约 200 MB，含内置视频壁纸）；
- 发布地址：<https://github.com/colm20060530/harness-desktop/releases/tag/v4.0.0>；
